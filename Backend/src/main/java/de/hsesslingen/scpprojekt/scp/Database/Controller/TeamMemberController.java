...
public class TeamMemberController {
    ...
    public ResponseEntity<TeamMember> addTeamMember(@RequestParam long TeamID, @RequestParam long MemberID,
        ...
        if (saml2Service.isLoggedIn(request)) {
            try {
                ...
                if (memberOptional.isPresent() && teamOptional.isPresent()) {
                    TeamMember newteamMember = teamMemberRepository
                            .save(new TeamMember(teamOptional.get(), memberOptional.get()));
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Team-Member found", content = {
    ...
    public ResponseEntity<List<TeamMember>> getAllTeamMembers(@PathVariable("id") long TeamID,
            HttpServletRequest request) {
        if (saml2Service.isLoggedIn(request)) {
            Optional<Team> teamOptional = teamRepository.findById(TeamID);
